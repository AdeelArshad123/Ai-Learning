...
export default function PersonalizedRecommendations() {
  ...
  useEffect(() => {
    ...
    const savedFavorites = localStorage.getItem('recommendation-favorites')
    const savedBookmarks = localStorage.getItem('recommendation-bookmarks')
  ...
  const filteredRecommendations = recommendations.filter(rec => {
    ...
    if (filter === 'favorites') return favorites.has(rec.id)
    if (filter === 'bookmarks') return bookmarks.has(rec.id)
    return rec.type === filter
  })

  const filterOptions = [
    { value: 'all', label: 'All', icon: FaStar },
    { value: 'favorites', label: 'Favorites', icon: FaHeart },
    { value: 'bookmarks', label: 'Bookmarks', icon: FaBookmark },
    { value: 'quiz', label: 'Quizzes', icon: FaBookOpen },
    { value: 'tutorial', label: 'Tutorials', icon: FaCode },
    { value: 'project', label: 'Projects', icon: FaRocket },
    { value: 'video', label: 'Videos', icon: FaVideo },
    { value: 'article', label: 'Articles', icon: FaNewspaper }
  ]
            ...
            {filterOptions.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  {option.value === 'bookmarks' && bookmarks.size > 0 && (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`p-2 rounded-full transition-colors ${
                            favorites.has(rec.id)
                              ...
                              : 'bg-gray-100 text-gray-400 hover:bg-red-100 hover:text-red-500 dark:bg-gray-700 dark:hover:bg-red-900/30'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`p-2 rounded-full transition-colors ${
                            bookmarks.has(rec.id)
                              ? 'bg-blue-100 text-blue-500 dark:bg-blue-900/30'
                              : 'bg-gray-100 text-gray-400 hover:bg-blue-100 hover:text-blue-500 dark:bg-gray-700 dark:hover:bg-blue-900/30'
                          }`}
                        >
